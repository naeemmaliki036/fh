...
import { ArrowLeft, BedDouble, Bath, Square, Building2, MapPin, Hammer } from "lucide-react";
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="truncate text-xs text-slate-500">
          {agent.license_id ? `License: ${agent.license_id}` : "Property Advisor"}
        </p>
      </div>
    </div>
  );
...
export function ListingDetailShell({ listing, slug }: ListingDetailShellProps): React.ReactElement {
  ...
  const floorPlanUrls = listing.floor_plan_urls ?? [];
  const [activeImage, setActiveImage] = useState(0);
  const { data: similarListings } = useSimilarListings(slug, listing.id, 4);

  const statCount =
    (listing.beds != null ? 1 : 0) + (listing.baths != null ? 1 : 0) +
    (listing.area_sqft != null ? 1 : 0) + 1 + (listing.build_year != null ? 1 : 0);
    ...
    <div className="min-h-screen bg-white">
      {listing.status !== "active" && (
        <div className="border-b border-amber-200 bg-amber-50 px-4 py-2.5 text-center text-sm text-amber-900">
          Preview mode — this listing is in <strong>{listing.status.replace(/_/g, " ")}</strong> state.
        </div>
      )}
      ...
      <div className="relative">
        <div className="relative h-[420px] overflow-hidden bg-slate-900">
          {urls.length > 0 ? (
            <img src={urls[activeImage]} alt={listing.title} className="absolute inset-0 h-full w-full object-cover" />
          ) : (
            <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, hsl(var(--primary)) 0%, hsl(var(--primary) / 0.7) 50%, #111827 100%)" }} />
          )}
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-black/10" />

          <Link href={`/p/${slug}`} className="absolute left-4 top-4 z-10 inline-flex items-center gap-1.5 text-sm font-medium text-white/85 transition hover:text-white">
          ...
          <div className="absolute right-4 top-4 z-10 hidden w-[340px] rounded-2xl bg-white/95 p-5 shadow-lg backdrop-blur-sm lg:block">
            ...
            {listing.agent ? <AgentCardCompact agent={listing.agent} /> : <ContactForm slug={slug} listingId={listing.id} compact />}
            {listing.agent?.phone && (
              <div className="mt-3">
          ...
          <div className="flex gap-3 overflow-x-auto px-6 py-4">
            {urls.map((url, i) => (
              <button key={i} onClick={() => setActiveImage(i)}
                className={`relative h-20 w-28 shrink-0 overflow-hidden rounded-xl border-2 transition-all ${i === activeImage ? "border-slate-950 ring-2 ring-slate-200" : "border-slate-200 opacity-70 hover:opacity-100"}`}>
      ...
      <header className="border-b border-slate-100 px-6 py-3">
        <div className="mx-auto w-[min(100%-0px,1280px)] flex items-center gap-2 text-sm text-slate-500">
          <Link href={`/p/${slug}`} className="hover:text-slate-900">Listings</Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_360px]">
          <div className="space-y-6">
            <div className={`grid grid-cols-2 gap-3 ${gridCols}`}>
              ...
              {listing.area_sqft != null && <StatTile icon={Square} value={`${listing.area_sqft.toLocaleString()} sqft`} label="Area" />}
              <StatTile icon={Building2} value={listing.property_type.replace(/_/g, " ")} label="Type" />
              {listing.build_year != null && <StatTile icon={Hammer} value={listing.build_year} label="Built" />}
            </div>
