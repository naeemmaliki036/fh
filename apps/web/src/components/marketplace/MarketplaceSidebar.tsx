...
export function MarketplaceSidebar({ onListWithUs }: MarketplaceSidebarProps): React.ReactElement {
  const { data } = useMarketplaceAgencies(1, 5);
  const topAgencies = data?.items ?? [];

  return (
    <aside className="flex flex-col gap-4 w-full">
      {/* Get listed promo card */}
      <div className="rounded-xl bg-slate-900 text-white p-5 space-y-3">
        ...
        <button
          ...
          className="w-full rounded-lg bg-amber-400 py-2 text-sm font-black text-slate-900 hover:bg-amber-300 transition"
        >
          Become a partner
        </button>
      </div>
